...
- **Popup denial**: New window requests (`window.open`, `target="_blank"`) to external domains are silently denied rather than opened in the browser, preventing ad popup spam. Legitimate external link navigation is handled separately by the navigation handler.
